...
local recoilAmount = Vector2.new(2, 1.5) -- Ajusta este valor según tu juego
...
mainFrame.Size = UDim2.new(0, 250, 0, 230) -- aumenté alto para botón extra
...
-- Dragging logic for mainFrame and logoImage
local dragging, offset
logoImage.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = true
        offset = Vector2.new(input.Position.X - logoImage.AbsolutePosition.X, input.Position.Y - logoImage.AbsolutePosition.Y)
    end
end)
...
local toggleRecoilButton = createButton(mainFrame, "Enable Anti-Recoil", UDim2.new(0.8, 0, 0, 30), UDim2.new(0.1, 0, 1.10, 0), Color3.fromRGB(100, 255, 100))

-- Ajustar altura del mainFrame para que quepa el nuevo botón
...
local function applyAntiRecoil()
    if antiRecoilEnabled and aimbotHold then
        -- Aplica el movimiento inverso para compensar el recoil
        mousemoverel(-recoilAmount.X, -recoilAmount.Y)
    end
end
...
toggleRecoilButton.MouseButton1Click:Connect(function()
    antiRecoilEnabled = not antiRecoilEnabled
    toggleRecoilButton.Text = antiRecoilEnabled and "Disable Anti-Recoil" or "Enable Anti-Recoil"
end)

-- Listen player join/leave for ESP
for _, player in ipairs(Players:GetPlayers()) do
